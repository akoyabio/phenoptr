...
# Color palette for nearest-neighbor plots
...
csd %>% dplyr::group_by(Phenotype) %>% 
  dplyr::summarize(Count=n(), 
                   Proportion=scales::percent(round(Count/nrow(csd), 2))) %>% 
...
```{r fig.height=6+2*(length(levels(csd$Phenotype))-1) %/% 3}
...
nn_plot_mutual(pheno_data1, pheno_data2, nn_mutual, background, xlim, ylim)
```
