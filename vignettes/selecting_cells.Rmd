...
an expression like ``~`Entire Cell PDL1 (Opal 620) Mean`>0.5``. 
In this example, the column name is `Entire Cell PDL1 (Opal 620) Mean`.

Expressions and phenotype names may be combined in a list. For example,
to select `tumor` cells with PDL1>0.5, use the parameter
...
For example, to pair `tumor` cells with `CD8` cells, use the argument
...
For multiple pairs, list each pair separately. For example, to pair `tumor` 
...
For example, to create a `T Cell` phenotype which matches `CD8` and `T reg`
phenotypes, and pair it with a `PDL1+ tumor` phenotype which 
...
from `PDL1+ tumor` to `CD68` cells, where `CD68` is an existing phenotype,
