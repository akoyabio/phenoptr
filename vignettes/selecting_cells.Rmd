...
can also be specified using `parse_phenotypes`, which supports a friendlier
syntax. 
</div></div>
...
selects the rows containing `CK+` cells. The same syntax works with both
`parse_phenotypes` and `select_rows`.
